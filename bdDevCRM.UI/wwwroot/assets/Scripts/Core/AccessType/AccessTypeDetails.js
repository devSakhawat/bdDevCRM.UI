...
var accessTypeDetailsManager = {
    ...
    SaveData: function () {
        ...
        var apiLink = isToUpdateOrCreate == 0 ? "/UsersAccessType/SaveUsersAccessType" : "/UsersAccessType/UpdateUsersAccessType";
        ...
        AjaxManager.MsgBox('information', 'center', 'Confirmation', confmsg,
            [{
                ...
                onClick: function ($noty) {
                    $noty.close();
                    var obj = accessTypeDetailsHelper.CreateNewObject();
                    var jsonObject = JSON.stringify(obj);
                    var msg = AjaxManager.PostDataForDotnetCore(coreManagement, apiLink, jsonObject);
                    if (msg === "Success") {
                        Message.Success(successmsg);
                        accessTypeDetailsHelper.CloseInformation();
...
var accessTypeDetailsHelper = {
    ...
    AddNewInformation: function () {
        ...
        accessTypeDetailsHelper.ClearInformation();

    },

    SaveInformation: function () {
        if (accessTypeDetailsHelper.validator()) {
            accessTypeDetailsManager.SaveData();
        }
    ...
    CreateNewObject: function () {
        ...
        var obj = new Object();
        obj.UsersAccessTypeId = $("#hdnId").val();
        obj.UsersAccessTypeCode = $("#txtCode").val();
        obj.UsersAccessTypeName = $("#txtName").val();
        obj.IsActive = $("#cmbStatus").data("kendoDropDownList").value();

       
        if (obj.UsersAccessTypeId == 0) {
            obj.CreatedBy = 1;
            obj.CreateDate = null;
            obj.UpdateBy = null;
            obj.UpdateDate = null;
    ...
    PopulateNewObject: function (obj) {
        ...
        $("#hdnId").val(obj.usersAccessTypeId);
        $("#txtCode").val(obj.usersAccessTypeCode);
        $("#txtName").val(obj.usersAccessTypeName);
        if (obj.updateBy > 0) {
